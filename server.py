...
# Android는 별도 계정 가입 없이 서버 인증키로 로그인합니다.
# iPhone의 Apple 로그인/세션 로직과 완전히 분리되어 기존 iPhone 동작에는 영향이 없습니다.
_android_session_serializer = URLSafeSerializer(SESSION_SECRET, salt="codenote-android-admin-session-v1")
_android_manage_serializer = URLSafeTimedSerializer(SESSION_SECRET, salt="codenote-android-admin-manage-v1")
ANDROID_MASTER_KEYS = {"codenote.kyh", "kyh"}
...
def issue_android_session(code: str) -> str:
    return _android_session_serializer.dumps({"provider": "android-key", "code": code})


def require_android_session_token(token: str) -> tuple[str, dict]:
    token = (token or "").strip()
    if not token:
        raise HTTPException(status_code=401, detail="android_login_required")
    try:
        payload = _android_session_serializer.loads(token)
    except BadSignature as exc:
        raise HTTPException(status_code=401, detail="invalid_android_session") from exc
...
def require_android_session(request: Request) -> tuple[str, dict]:
    return require_android_session_token(request.headers.get("X-Android-Session", ""))


def issue_android_manage_token(code: str) -> str:
    return _android_manage_serializer.dumps({"code": code})


def require_android_manage_token(request: Request, code: str):
    token = request.headers.get("X-Manage-Token", "")
    if not token:
        raise HTTPException(status_code=401, detail="manage_access_required")
    try:
        payload = _android_manage_serializer.loads(token, max_age=60 * 15)
    except (BadSignature, SignatureExpired) as exc:
        raise HTTPException(status_code=401, detail="manage_access_expired") from exc
...
# ============================================================
#   Android 인증키 로그인 / 업로드 권한 API
#   기존 Apple API와 분리되어 iPhone 동작은 그대로 유지됩니다.
# ============================================================
...
def android_admin_session(request: Request):
    _, profile = require_android_session(request)
    return {"status": "ok", "profile": profile}


@app.get("/android-admin/upload-categories")
def android_admin_upload_categories(request: Request):
    _, profile = require_android_session(request)
    allowed = profile.get("allowedCategory")
    if allowed == "전체":
        return {
...
@app.post("/android-admin/upload-categories")
def android_admin_add_upload_category(req: CategoryRequest, request: Request):
    _, profile = require_android_session(request)
    if profile.get("allowedCategory") != "전체":
        raise HTTPException(status_code=403, detail="category_add_not_allowed")
    name = clean_category(req.name)
...
def android_admin_manage_access(req: AppleAdminManageAccessRequest, request: Request):
    source_code, _ = require_android_session(request)
    validate_and_consume_kyh_code(req.code)
    return {"status": "ok", "manageToken": issue_android_manage_token(source_code)}


@app.get("/android-admin/apple-admins")
def android_apple_admin_list(request: Request):
    source_code, _ = require_android_session(request)
    require_android_manage_token(request, source_code)
    with _db_lock:
        items = [apple_admin_profile(uid) for uid in apple_admins.keys()]
    items.sort(key=lambda x: x.get("registeredAt") or "", reverse=True)
    return {"items": items}
...
@app.post("/android-admin/apple-admins/update")
def android_apple_admin_update(req: AppleAdminUpdateRequest, request: Request):
    source_code, _ = require_android_session(request)
    require_android_manage_token(request, source_code)
    label = (req.label or "").strip()
    if not label:
...
@app.post("/android-admin/apple-admins/delete")
def android_apple_admin_delete(req: AppleAdminDeleteRequest, request: Request):
    source_code, _ = require_android_session(request)
    require_android_manage_token(request, source_code)
    with _db_lock:
        if req.userId not in apple_admins:
